#ifndef FIVO_LINEAR_ELASTIC_PWAVES_H
#define FIVO_LINEAR_ELASTIC_PWAVES_H
...
class LinearElasticPWaves : public SystemBase<fivo::state<double, 2>> {
public:
  ...
  struct Params { value_type rho, lambda, mu; };

  LinearElasticPWaves(Mesh const& mesh,
                      std::shared_ptr<BC> const& left_bc,
                      std::shared_ptr<BC> const& right_bc,
                      Params const& params)
    : base_type(mesh, left_bc, right_bc), m_rho(params.rho),
      m_lambda(params.lambda), m_mu(params.mu) {}

  auto get_params() const { return Params{m_rho, m_lambda, m_mu}; }
  ...
  flux_type flux(state_type const& s) const override {
    return flux_type{{{{- s[1]}, {- (m_lambda + 2 * m_mu) / m_rho * s[0]}}}};
  }
...
#endif // FIVO_LINEAR_ELASTIC_PWAVES_H
